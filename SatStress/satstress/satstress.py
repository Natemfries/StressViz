...
def eigen2(a, b, c):
    ...
    """
    sqrt_thing = np.sqrt(abs(a*a + 4.0*b*b - 2.0*a*c + c*c))
    ...
    mag1 = np.sqrt(eig1theta**2 + eig1phi**2)
    mag2 = np.sqrt(eig2theta**2 + eig2phi**2)
    ...
    return(np.array([lambda1, eig1theta, eig1phi, lambda2, eig2theta, eig2phi]))
...
class Satellite(object):
    ...
    def mass(self):
        ...
        for layer in self.layers:
            mass += ((4.0/3.0)*np.pi)*((radius+layer.thickness)**3 - (radius**3))*layer.density
    ...
    def orbit_period(self):
        ...
        return(2.0*np.pi*np.sqrt( (self.orbit_semimajor_axis**3) / (pc.G*self.planet_mass) ))
...
def avg_body_density(satellite):
    ...
    for l in satellite.layers[0:1]:
        v = 4.0 / 3.0 * np.pi * (np.power(r + l.thickness, 3) - np.power(r, 3))
        m += l.density * v
        r += l.thickness
    return m / (4.0 / 3.0 * np.pi * np.power(r, 3))
...
class LoveNum(object): #
    ...
    def update_h2(self, values):
        real, imag = values
        self.h2 = real + imag*1.0j

    def update_k2(self, values):
        real, imag = values
        self.k2 = real + imag*1.0j

    def update_l2(self, values):
        real, imag = values
...
class PW_Variables(object): #
  ...
  def __init__(self, thetaRi=0, phiRi=0, thetaRf=0, phiRf=0, thetaTi=0, phiTi=0, thetaTf=0, phiTf=0, locked=True, despinning=False, InitialSpinPeriod=0, FinalSpinPeriod=0):
    self.thetaRInitial = np.pi/2 - np.radians(thetaRi) # Converts from latitude to colatitude
    self.phiRInitial = np.radians(phiRi)
    self.thetaRFinal = np.pi/2 - np.radians(thetaRf)
    self.phiRFinal = np.radians(phiRf)

    self.locked = locked
    #If the satellite is tidally locked, the rotational axis and tidal axis will always be at a right angle to each other.
    if self.locked:
      self.thetaTInitial = self.thetaRInitial + np.pi/2
      self.thetaTFinal = self.thetaRFinal + np.pi/2
      self.phiTInitial = self.phiRInitial
      self.phiTFinal = self.phiRFinal
    else:
      self.thetaTInitial = np.pi/2 - np.radians(thetaTi)
      self.phiTInitial = np.radians(phiTi)
      self.thetaTFinal = np.pi/2 - np.radians(thetaTf)
      self.phiTFinal = np.radians(phiTf)

    self.despinning = despinning
    if self.despinning:
      self.InitialSpin = (2*np.pi) / (InitialSpinPeriod * 60 * 60)
      self.FinalSpin = (2*snp.pi) / (FinalSpinPeriod * 60 * 60)
  ...
  def update_thetaRi(self, thetaRi):
    self.thetaRInitial = np.pi/2 - np.radians(thetaRi)
    if self.locked:
      self.thetaTInitial = self.thetaRInitial + np.pi/2
  ...
  def update_FinalSpin(self, FinalSpinPeriod):
    self.FinalSpin = 2 * np.pi / (FinalSpinPeriod * 60 * 60)
...
class StressDef(object): #{{{
    ...
    def __init__(self):
        self.omega = 0.0
        self.satellite = None
        self.love = LoveNum(0, 0, 0, 0, 0, 0,)
        self.dependson = set(['DENSITY',\
                        'YOUNGS_MODULUS',\
                        'POISSONS_RATIO',\
                        'THICKNESS',\
                        'VISCOSITY'])
...
""" % (self.__name__, self.Delta(),\
    ...
    def calcLove(self): #
        self.calcLoveNew() #call calcLoveNew() for consistency
        """
    ...
    def calcLoveNew(self):
      #Python version of the Love code (use this!)

      if self.omega == 0.0:
          self.calcLoveInfinitePeriod()
      else:
          param = np.zeros((6,4),'complex')
          ...
          param[1,:] = np.array([1, 0, 1, 1])
          param[2,:] = np.array([self.satellite.layers[0].density, self.satellite.layers[1].density, self.satellite.layers[2].density, self.satellite.layers[3].density])
          #Treat the first two layers as elastic.
          param[3,:] = np.array([self.satellite.layers[0].lame_mu, self.satellite.layers[1].lame_mu, self.mu_twiddle(2), self.mu_twiddle(3)])
          ...
          param[4,:] = np.array([self.satellite.layers[0].lame_lambda, 2e+9, self.lambda_twiddle(2), self.lambda_twiddle(3)])
          
          [h2Re,h2Im,k2Re,k2Im,l2Re,l2Im] = love3.thirdLove(10,param,self.omega)
          self.love = LoveNum(h2Re,h2Im,k2Re,k2Im,l2Re,l2Im)
          print("===Debug in calcLoveNew: assigned LoveNum ===")
          print("h2:", self.love.h2, "k2:", self.love.k2, "l2:", self.love.l2)
...
class NSR(StressDef): #{{{
    ...
    def __init__(self, satellite):
        ...
        # of only 2*pi.
        self.omega = 4.0*np.pi/self.satellite.nsr_period
    ...
    def Ttt(self, theta, phi, t):
        ...
        if self.omega == 0:
            return(0.0)
        TttN = (self.b1()-self.g1()*np.cos(2.0*theta))*np.exp(1j*(2.0*phi+self.omega*t))
        TttN = TttN.real
    ...
    def Tpp(self, theta, phi, t):
        ...
        if self.omega == 0:
            return(0.0)
        TppN = (self.b2()-self.g2()*np.cos(2.0*theta))*np.exp(1j*(2.0*phi+self.omega*t))
        TppN = TppN.real
    ...
    def Tpt(self, theta, phi, t):
        ...
        if self.omega == 0:
            return(0.0)
        TptN = self.Gamma()*1j*np.exp(1j*(2.0*phi+self.omega*t))*np.cos(theta)
        TptN = TptN.real
...
class Diurnal(StressDef): #{{{
    ...
    def Ttt(self, theta, phi, t):
        ...
        stress tensor.
        """
        Ttt1 = 3.0*(self.b1()-self.g1()*np.cos(2.0*theta))*np.exp(1j*self.omega*t)*np.cos(2.0*phi)
        Ttt2 = -1.0*(self.b1()+3.0*self.g1()*np.cos(2.0*theta))*np.exp(1j*self.omega*t)
        Ttt3 = -4.0*(self.b1()-self.g1()*np.cos(2.0*theta))*1j*np.exp(1j*self.omega*t)*np.sin(2.0*phi)
        TttD = Ttt1 + Ttt2 + Ttt3
    ...
    def Tpp(self, theta, phi, t):
        ...
        Calculates the S{tau}_S{phi}S{phi} (east-west) component of the stress tensor.
        """
        Tpp1 = 3.0*(self.b2()-self.g2()*np.cos(2.0*theta))*np.exp(1j*self.omega*t)*np.cos(2.0*phi)
        Tpp2 = -1.0*(self.b2()+3.0*self.g2()*np.cos(2.0*theta))*np.exp(1j*self.omega*t)
        Tpp3 = -4.0*(self.b2()-self.g2()*np.cos(2.0*theta))*1j*np.exp(1j*self.omega*t)*np.sin(2.0*phi)
        TppD = Tpp1 + Tpp2 + Tpp3
    ...
    def Tpt(self, theta, phi, t):
        ...
        stress tensor.
        """
        Tpt1 = -4.0*self.Gamma()*1j*np.exp(1j*(self.omega*t))*np.cos(theta)*np.cos(2.0*phi)
        Tpt2 = -3.0*self.Gamma()*np.exp(1j*self.omega*t)*np.cos(theta)*np.sin(2.0*phi)
        TptD = Tpt1 + Tpt2
...
class DiurnalObliquity(StressDef): #{{{
    ...
    def Ttt(self, theta, phi, t):
        ...
        TttDO = self.g1()*np.exp(1j*(self.periapsis_arg + self.omega*t))*np.sin(2.0*theta)*np.cos(phi)
        TttDO = TttDO.imag*np.sin(2.0*self.eps)*self.Z()/(self.satellite.surface_gravity()*self.satellite.radius())
    ...
    def Tpp(self, theta, phi, t):
        ...
        TppDO = self.g2()*np.exp(1j*(self.periapsis_arg + self.omega*t))*np.sin(2.0*theta)*np.cos(phi)
        TppDO = TppDO.imag*np.sin(2.0*self.eps)*self.Z()/(self.satellite.surface_gravity()*self.satellite.radius())
    ...
    def Tpt(self, theta, phi, t):
        ...
        TptDO = self.Gamma()*np.exp(1j*(self.periapsis_arg + self.omega*t))*np.sin(theta)*np.sin(phi)
        TptDO = TptDO.imag*np.sin(2.0*self.eps)*2.0*self.Z()/(self.satellite.surface_gravity()*self.satellite.radius())
...
class PolarWander(StressDef): #{{{
    ...
    def CosGamma(self, theta, phi, theta_R, phi_R):
        ...
        """
        return np.cos(theta) * np.cos(theta_R) + np.sin(theta) * np.sin(theta_R) * np.cos(phi - phi_R)
        
    def SinGammaCosPsi(self, theta, phi, theta_R, phi_R):
        """
        One of the spherical transformations for calculating Polar Wander Stress
        """
        return np.cos(theta_R) * np.sin(theta) - np.sin(theta_R) * np.cos(theta) * np.cos(phi - phi_R)
        
    def SinGammaSinPsi(self, theta, phi, theta_R, phi_R):
        """
        One of the spherical transformations for calculating Polar Wander Stress
        """
        return np.sin(theta_R) * np.sin(phi - phi_R)
    ...
    def Ttt(self, theta, phi, t):
        ...
        """
        TttPW = self.PWC * (1.0/2.0) * np.sin(theta_R) * 4.0 * np.cos(theta_R) * np.sin(2.0 * theta) * np.cos(phi) - np.sin(theta_R) * np.cos(2.0 * theta) * (3.0 + np.cos(2.0 * phi)) + 10.0 * (np.sin(phi))**2
    ...
    def Tpp(self, theta, phi, t):
        ...
        """
        TppPW = self.PWC * (1.0/2.0) * np.sin(theta_R) * (np.sin(phi))**2 + 12.0 * np.cos(theta_R) * np.sin(2.0*theta) * np.cos(phi) - 3.0 * np.sin(theta_R) * np.cos(2.0 * theta) * (3 + np.cos(2.0 * phi))
    ...
    def Tpt(self, theta, phi, t):
        ...
        """
        TptPW = self.PWC * 4.0 * np.sin(theta_R) * np.sin(phi) * (np.sin(theta_R) * np.cos(theta) * np.cos(phi) - np.cos(theta_R) * np.sin(theta))
...
class IST(StressDef): #{{{
    ...
    def Ttt(self, theta, phi, t):
        ...
        """
        return np.ones(np.shape(theta)) * self.satellite.layers[3].youngs_modulus * self.delta_tc * \
        ...
        """The above is an elastic expression. The viscoelastic expression below has not been verified, so 
        should not be implemented until it can be trusted."""    
        ...
        if self.diffusivity == 0:
            ...
            return np.ones(np.shape(theta)) * delta_rho / ( self.satellite.layers[3].density * self.satellite.radius()) * \
                np.sqrt( 6.0 * self.satellite.layers[3].youngs_modulus * self.satellite.layers[3].viscosity / ( 1.0 - self.satellite.layers[3].poissons_ratio ) ) * \
                2.0*self.lam_1*np.sqrt(self.diffusivity) * \
                scipy.special.dawsn( self.delta_tc / ( 2.0 * self.lam_1 * np.sqrt(self.diffusivity) ) * np.sqrt( self.satellite.layers[3].youngs_modulus / ( 6.0 * self.satellite.layers[3].viscosity * ( 1.0 - self.satellite.layers[3].poissons_ratio ) ) ) )
    """
...
class StressCalc(object): #{{{
    ...
    def tensor(self, theta, phi, t): #{{{2
        ...
        Ttt = np.array( [ stress.Ttt(theta, phi, t) for stress in self.stresses ] )
        Tpp = np.array( [ stress.Tpp(theta, phi, t) for stress in self.stresses ] )
        Tpt = np.array( [ stress.Tpt(theta, phi, t) for stress in self.stresses ] )
    ...
    def principal_components(self, theta, phi, t): #{{{2
        ...
        az_A = np.mod(np.arctan2(phi_A,-theta_A), np.pi)
        az_B = np.mod(np.arctan2(phi_B,-theta_B), np.pi)

        tens_mag = np.where(eigval_A >  eigval_B, eigval_A, eigval_B)
        tens_az  = np.where(eigval_A >  eigval_B, az_A, az_B)
        comp_mag = np.where(eigval_A <= eigval_B, eigval_A, eigval_B)
        comp_az  = np.where(eigval_A <= eigval_B, az_A, az_B)
...
def random_loncolatpoints(N): #{{{
    ...
    """
    return(2*np.pi*np.random.rand(N), np.arccos(2*np.random.rand(N)-1))
#}}}
...
Recall that all input parameters are in SI (meters, kilograms, seconds) units.
""" % (self.n, float(self.sat.satParams['THICKNESS_'+str(self.n)]), self.sat.sourcefilename))
