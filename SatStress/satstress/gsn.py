...
class GeoSupNet(nx.MultiDiGraph): #{{{
    ...
    def to_shp(self, lin_shp_file, cross_shp_file): #{{{2 # TODO: write it!
        """
    ...
    def reorder(self, linstack): #{{{2 TODO
        """
    ...
    def draw_sort(self, orderby='mean', ax=None, colorby='graph', cmap=pylab.cm.jet_r, title="", weighted=True, normalized=True, minsize=2, minconf=0.5): #{{{2 TODO: Borken!
        """
    ...
    def draw_idist(self, iterations=100, minsize=4, minconf=4, cmap=pylab.cm.jet_r): #{{{2 TODO: Borken!
        """
        ...
        for idist,sub_gsn,nc in zip(GIDs, sub_GSNs, nc_array):
            n, bins, patches = hist_ax.hist(idist, facecolor=nc, density=True, histtype='stepfilled', lw=2, edgecolor=nc, alpha=0.5)
...
def test(lintype='nsr', nlins=300, nbins=9, minconf=0.5, maxconf=1.0,\
   ...
   if draw_sort is True:
       the_gsn.draw_sort(orderby=orderby, title=lintype, minsize=minsize)
   if draw_idist is True:
       the_gsn.draw_idist(minsize=minsize, minconf=minconf, iterations=iterations)
...
def GSN_Map(the_gsn, label='GSN', min_cross_conf=0.7, K_min=0.0): #{{{
    ...
    e15_map_ax = map_cycle_fig.axes[0]
...
def Intersection_Confs(the_gsn, label='Mapped'): #{{{
    ...
    half_fives = int(len(int_confs[np.where(int_confs == 0.5)])/2)
...
def GSN_vs_NSR(the_gsn, iters=100, nb=180, nbins=180, linfile=None, label='E15'): #{{{
    ...
    x_min,x_max = (0.3,0.8)
    hist_ax = the_fig.add_subplot(1,1,1)
    hist_ax.hist(A_rand, facecolor='gray', linewidth=0, bins=int(len(A_rand)/20), range=(x_min,x_max), density=True)
    hist_ax.grid()
