...
def tpw_polesearch(satfile="input/ConvectingEuropa_GlobalDiurnalNSR.ssrun",\
    ...
    for pnp_lon, pnp_lat, N in zip(pnp_lons, pnp_lats, arange(len(pnp_lons))):
        ...
        if nlins == 0:
            with open(os.path.join(outdir,poledir,label+'.pkl'), 'wb') as f:
                pickle.dump(tpwlins, f)
...
def linresample_byN(lins, fraction=1.0): #{{{
    ...
    return(array(lins)[np.random.randint(len(lins), size=int(fraction*len(lins)))])
#}}} end linresample
...
def linresample_byL(lins, fraction=1.0): #{{{
    ...
    for i in range(len(linlengths)):
        lookup_list += [i,]*int(mincount*linlengths[i])
...
def load_lins(linpath): #{{{
    ...
    if os.path.isdir(linpath):
        ...
        for linfilename in lindir:
            ...
            with open(linfilepartname, 'rb') as f:
                newpart = pickle.load(f)
            print("  found %d lineaments" % (len(newpart,)))
            linlist += newpart
    elif os.path.isfile(linpath):
        with open(linpath, 'rb') as f:
            linlist = pickle.load(f)
    else:
...
def ActHist_RMDProb(maplins, tpwlins): #{{{
    ...
    with open('output/spin_map_acthists', 'rb') as f:
        spin_acthists = pickle.load(f)
    spin_nb = len(spin_acthists[0])
    spin_RMDs = [ np.fabs(np.tile(ah,len(ah)) - np.repeat(ah,len(ah))).mean()/ah.mean() for ah in spin_acthists ]

    with open(os.path.join(outdir, 'jack_map_acthists'), 'rb') as f:
        jack_acthists = pickle.load(f)
    ...
    spin_counts, spin_bins, spin_patches = hist_ax.hist(spin_RMDs, bins=96, range=(0.0,0.6), color='red',   lw=2, density=True, histtype='step', label='spin (N=%d)' % (len(spin_RMDs),))
    jack_counts, jack_bins, jack_patches = hist_ax.hist(jack_RMDs, bins=48,  range=(0.0,0.6), color='green', lw=2, density=True, histtype='step', label='jack (N=%d)' % (len(jack_RMDs),))
    pnp_counts,  pnp_bins,  pnp_patches  = hist_ax.hist(pnp_RMDs,  bins=48,  range=(0.0,0.6), color='blue',  lw=2, density=True, histtype='step', label='TPW (N=%d)'  % (len(pnp_RMDs),))
...
def LinLatStatsCompare(maplins, synlins=None, bins=30, fold=False): #{{{
    ...
    gc_n, gc_bins = np.histogram(gc_lats, bins=bins, range=(min_lat,max_lat), density=True, weights=1./np.cos(gc_lats))
    ...
    map_n, map_bins = np.histogram(map_lats, bins=bins, range=(min_lat, max_lat), density=True, weights=map_lens/np.cos(map_lats))
    ...
    syn_n, syn_bins = np.histogram(syn_lats, bins=bins, range=(min_lat,max_lat), density=True, weights=syn_lens/np.cos(syn_lats))
...
def activity_history(lins_list, the_fig=None, labels=[], colors=[], alphas=[], lin_cm=cm.jet, norm_by_all=False, dbar_max=0.125, verbose=True, titlestr="Apparent Lineament Activity History", outfile=None): #{{{
    ...
    if (len(labels) == 0):
        labels = [ 'Series '+str(N) for N in np.arange(len(lins_list))+1 ]
    try:
