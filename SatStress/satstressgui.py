...
#     tempfile = allows create a temp file while program is running
import copy, sys, os, tempfile
...
class SatelliteCalculation(object):
    ...
    stress_d = {
        'Nonsynchronous Rotation': NSR,
        'Diurnal': Diurnal,
        'Ice Shell Thickening': IST,
        'Obliquity': DiurnalObliquity,
        'Polar Wander': PolarWander}
    ...
    def save_satellite(self, filename=None):
        ...
        if not filename:
            tmp = True
            with tempfile.NamedTemporaryFile(prefix='sat', delete=False, mode='w') as f:
                filename = f.name # Saves to the temporary directory.  This is necessary b/c of how satstress.py handles reading in the variables.  -PS 2016
                t = self.parameters['NSR_PERIOD']
                self.nsr_period_years2seconds()
                f.write(self.dump_satellite())
                self.parameters['NSR_PERIOD'] = t # Restore original value
        else:        
    ...
    def get_stresses(self):
        ...
        print("parameters dict:", self.parameters)
        print("enabled stresses:", [v for v in self.stress_d if self.parameters.get(v)])
        print("created stress objects:", self.stresses)
    ...
    def save_grid(self, filename=None):
        tmp = False
        if filename is None:
            fd, filename = tempfile.mkstemp(prefix='grid')
            tmp = True
        f = open(filename, 'w')
        try:
    ...
    def mk_grid(self):
        if getattr(self, "mode", None) == "point":
            print("Skipping grid creation (point mode active)")
            return None
    ...
    def load_netcdf_satellite(self, nc):
        fd, satfilename = tempfile.mkstemp(prefix='sat')
    ...
    def load_netcdf_grid(self, nc):
        fd, gridfilename = tempfile.mkstemp(prefix='grid')
...
# ===============================================================================
def file_dir_dialog(parent, dialog_class, message="", style=wx.FD_OPEN, action=None, **kw):
...
def file_dialog(parent, message="", style=wx.FD_OPEN, action=None, **kw):
    file_dir_dialog(parent, wx.FileDialog, message, style, action, **kw)

def dir_dialog(parent, message="", style=wx.FD_OPEN, action=None, **kw):
...
def add_checkboxes_to_sizer(parent, sz, parameters_d):
    ...
    for p, d in parameters_d:
        ...
        sz.Add(parameters[p], flag=wx.ALIGN_CENTER_HORIZONTAL)
    return parameters
...
class SatelliteLayersPanel(SatPanel):
    ...
    def __init__(self, *args, **kw):
        ...
        sp = wx.FlexGridSizer(0, 2, 5, 5)
        self.parameters = add_parameters_to_sizer(self, sp, self.sc.satellite_vars)
        # layers parameters
        lp = wx.FlexGridSizer(0, len(self.sc.layer_vars_d), 5, 5)
...
class StressListPanel(SatPanel):
    ...
    def __init__(self, *args, **kw):
        ...
        # "Input Love Numbers" toggle (main one above grid)
        self.inputLoveToggle = wx.CheckBox(self, label="Input Love Numbers")
        self.Bind(wx.EVT_CHECKBOX, self.toggle_love_inputs, self.inputLoveToggle)

        # New checkbox (auto Love number toggle)
        self.calcLoveToggle = wx.CheckBox(self, label="Calculate Love Numbers Automatically")
        self.Bind(wx.EVT_CHECKBOX, self.toggle_calc_love, self.calcLoveToggle)
        ...
        self.userDiurn = wx.CheckBox(self, label='Diurnal')
        self.userNSR = wx.CheckBox(self, label='Nonsynchronous Rotation')

        # Add to parameters dict
        self.parameters['Diurnal'] = self.userDiurn
        self.parameters['Nonsynchronous Rotation'] = self.userNSR

        # Bind stress toggle checkboxes
        self.Bind(wx.EVT_CHECKBOX, self.on_set_diurn, self.userDiurn)
        self.Bind(wx.EVT_CHECKBOX, self.on_set_nsr, self.userNSR)

        # Force stress dict keys to exist (default to False)
        self.sc.set_parameter('Diurnal', False)
        self.sc.set_parameter('Nonsynchronous Rotation', False)
        print("Stress keys:", self.sc.stress_d.keys())
        ...
        self.diurnalLabel = self.userDiurn
        self.nsrLabel = self.userNSR



        # Diurnal input fields
        self.h2Diurn = wx.TextCtrl(self, style=wx.TE_PROCESS_ENTER)
        self.k2Diurn = wx.TextCtrl(self, style=wx.TE_PROCESS_ENTER)
        self.l2Diurn = wx.TextCtrl(self, style=wx.TE_PROCESS_ENTER)
        ...
        # NSR input fields
        self.h2NSR = wx.TextCtrl(self, style=wx.TE_PROCESS_ENTER)
        self.k2NSR = wx.TextCtrl(self, style=wx.TE_PROCESS_ENTER)
        self.l2NSR = wx.TextCtrl(self, style=wx.TE_PROCESS_ENTER)
        ...
        # Love number grid (labels on left, h2/k2/l2 in 3 columns)
        loveGrid = wx.FlexGridSizer(rows=3, cols=4, hgap=5, vgap=5)
        loveGrid.AddSpacer(1)  # Empty top-left
        loveGrid.Add(self.h2)
        loveGrid.Add(self.k2)
        loveGrid.Add(self.l2)
        loveGrid.Add(self.diurnalLabel)
        loveGrid.Add(self.h2Diurn)
        loveGrid.Add(self.k2Diurn)
        loveGrid.Add(self.l2Diurn)
        loveGrid.Add(self.nsrLabel)
        loveGrid.Add(self.h2NSR)
        loveGrid.Add(self.k2NSR)
        loveGrid.Add(self.l2NSR)

        # Combine toggle + grid into one vertical section
        loveSection = wx.BoxSizer(wx.VERTICAL)
        loveSection.Add(self.inputLoveToggle, 0, wx.LEFT | wx.BOTTOM, 5)
        loveSection.Add(self.calcLoveToggle, 0, wx.LEFT | wx.BOTTOM, 5)
        loveSection.Add(loveGrid, 0, wx.LEFT, 20)

        # Add to main layout
        sz.AddSpacer(10)
        sz.Add(loveSection, 0, wx.LEFT, 20)
        ...
        #This feature isn't currently set up. For now, just enter Love numbers manually
        ...
        Polargrid = wx.FlexGridSizer(9, 3, 3, 5) # A GridSizer to hold the polar wander coordinates.  -PS 2016
    ...
    def toggle_calc_love(self, event):
        if self.calcLoveToggle.GetValue():
            self.inputLoveToggle.SetValue(False)
            self.disable_love_inputs()
            try:
                # 1) Ensure we have a Satellite object with layers
                sat = self.model.get_satellite()   # builds from Satellite tab / .sat file
                if sat is None or not getattr(sat, "layers", None):
                    wx.MessageBox(
                        "Please load a .sat file or fill in the Satellite tab first.",
                        "Satellite not defined",
                        wx.OK | wx.ICON_WARNING
                    )
                    return

                # 2) Attach to StressDef and ensure ω is nonzero (use real value later)
                sd = self.model.stressdef
                sd.satellite = sat
                sd.omega = sd.omega or (2 * np.pi / (3.551 * 86400.0))  # Europa diurnal default

                # 3) Compute Love numbers (Python path)
                sd.calcLoveNew()

                # Make sure Diurnal is enabled in the model + UI
                self.userDiurn.SetValue(True)
                self.sc.set_parameter('Diurnal', True)

                # Build/refresh the stress INSTANCES
                stresses = self.sc.get_stresses()

                # Find the Diurnal instance
                diurn = next((s for s in stresses if s.__class__.__name__ == "Diurnal"), None)
                if diurn is None:
                    wx.MessageBox("Could not create Diurnal stress instance.", "Error",
                                wx.OK | wx.ICON_ERROR)
                    return

                # Push Love numbers + ω into the Diurnal instance
                diurn.love = sd.love
                diurn.omega = sd.omega
                diurn.useUser = False  # don’t read manual boxes

                # Force recompute downstream
                self.sc.calc = None
                self.sc.stresses_changed = True


                # 4) Format safely and set WITHOUT firing EVT_TEXT
                def _fmt_complex(z):
                    s = '+' if z.imag >= 0 else '-'
                    return f"{z.real:.6f} {s} {abs(z.imag):.6f}j"

                love = sd.love
                self.h2Diurn.ChangeValue(_fmt_complex(love.h2))
                self.k2Diurn.ChangeValue(_fmt_complex(love.k2))
                self.l2Diurn.ChangeValue(_fmt_complex(love.l2))

                # 1) ensure Diurnal is enabled in the model + UI
                self.userDiurn.SetValue(True)
                self.sc.set_parameter('Diurnal', True)

                # 2) build/refresh the stress INSTANCES now
                stresses = self.sc.get_stresses()

                # 3) push Love numbers + ω into the Diurnal instance(s)
                for s in stresses:
                    if s.__class__.__name__ == "Diurnal":
                        s.love = sd.love
                        s.omega = sd.omega
                        s.useUser = False  # don't read manual text boxes

                # 4) force recompute downstream
                self.sc.calc = None
                self.sc.stresses_changed = True
                ...
                # 2) propagate the computed Love numbers & frequency into the stress object
                diurn.love    = sd.love
                diurn.omega   = sd.omega
                diurn.useUser = False   # make sure it doesn't read the manual boxes

                # 3) force a recompute downstream
                self.sc.stresses_changed = True
                self.sc.calc = None


                wx.MessageBox("Love numbers calculated from internal model.",
                            "Success", wx.OK | wx.ICON_INFORMATION)

            except Exception as e:
                import traceback; traceback.print_exc()
                wx.MessageBox(f"Love number calculation failed:\n{e}",
                            "Error", wx.OK | wx.ICON_ERROR)
        else:
            if not self.inputLoveToggle.GetValue():
                self.disable_love_inputs()




    def toggle_love_inputs(self, event):
        if self.inputLoveToggle.GetValue():
            self.calcLoveToggle.SetValue(False)
            self.enable_love_inputs()
        else:
            self.disable_love_inputs()
    ...
    def enable_love_inputs(self):
        # manual entry mode: editable
        for tc in (self.h2Diurn, self.k2Diurn, self.l2Diurn):
            tc.Enable(True)
            tc.SetEditable(True)

    def disable_love_inputs(self):
        # auto mode: visible but read-only (not greyed)
        for tc in (self.h2Diurn, self.k2Diurn, self.l2Diurn):
            tc.Enable(True)          # keep dark
            tc.SetEditable(False)    # read-only




    def disable_display_diurnlove(self):
        for widg in [self.h2Diurn, self.k2Diurn, self.l2Diurn,
                     self.userDiurn]:
            widg.Disable()
    ...
    def disable_display_nsrlove(self):
        for widg in [self.h2NSR, self.k2NSR, self.l2NSR,
                     self.userNSR]:
            widg.Disable()
...
class PointPanel(SatPanel):
    ...
    def params_grid(self, panel, params_d, defval, width=3, row = 1):
        pp = wx.FlexGridSizer(0, int(width), 5, 5)
    ...
    def __init__(self, *args, **kw):
        ...
        sz.AddSpacer(20)
        self.fieldPanel = scrolled.ScrolledPanel(self,-1, size=(1000,400), style=wx.SIMPLE_BORDER)
    ...
    def save_pointcalc(self, filename=None):
        tmp = False
        if not filename:
            fd, filename = tempfile.mkstemp(prefix='csv')
            tmp = True
        f = open(filename, 'wb')
        writer = csv.writer(f)
...
class GridCalcPanel(SatPanel):
    ...
    def __init__(self, *args, **kw):
        ...
        gmcp = wx.FlexGridSizer(0, len(self.sc.grid_vars_d) + 1, 5, 5)
        # grid points
...
class CycloidsPanel(SatPanel):
    ...
    def __init__(self, *args, **kw):
        ...
        gridSizer    = wx.FlexGridSizer(0, 0, 5, 0)
    ...
    def add_text_control(self,  sz, parameters_d):
        txtCtrls = {}
        for p, d in parameters_d:
            sz.Add(wx.StaticText(self, label=d), flag=wx.ALIGN_CENTER_HORIZONTAL)
...
class PlotPanel(SatPanel):
    ...
    def add_stepspin(self, sz):
        sz.Add(wx.StaticText(self, label=u"Tick mark increment"), flag=wx.ALIGN_CENTER_HORIZONTAL)
        self.stepspin = wx.SpinCtrl(self, initial=int(self.step), min=0, max=180)
        sz.Add(self.stepspin, flag=wx.ALL|wx.EXPAND)
        self.stepspin.SetValue(int(self.step))
    ...
    def plot_no_draw(self):
        ...
        if not self.grid:
            print(">>> No grid available - skipping plot")
            return # Prevent crash in point mode
...
class ScalarPlotPanel(PlotPanel):
    ...
    def parameters_sizer(self):
        ...
        lp.Add(self.pw_marker_box, 0, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL)
    ...
    def load_save_buttons(self):
        ...
        """
        gridSizer = wx.FlexGridSizer(2, 2, 15, 5)
        ...
        # add widgets to grid
        gridSizer.AddMany([
            (shapeLoad, 0, wx.EXPAND),
            (shapeSave, 0, wx.EXPAND),
            (netLoad, 0, wx.EXPAND),
            (netSave, 0, wx.EXPAND)])
    ...
    def lineaments_sizer(self):
        ...
        lins_ckSizer.Add(self.l_count_tc, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL)
        lins_ckSizer.Add(wx.StaticText(self, label=u" Lineaments"), wx.ALL|wx.ALIGN_CENTER_HORIZONTAL)
    ...
    def cycloids_sizer(self):
        ...
        # wrap in sizer
        ckSizer.Add(self.plot_cycl, 0, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL)
        # bind to event
        ...
        ckSizer.Add(self.plot_triangles, 0, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL)
        ...
        ckSizer.Add(self.cycl_names_cb , 0, wx.ALL|wx.ALIGN_CENTER_HORIZONTAL)
    ...
    def plot_cycloids(self):
        if self.sc.parameters['to_plot_many_cycloids']:
            for i, cycloid_params in enumerate(self.sc.params_for_cycloids.items()):
                ...
                if not i in self.sc.cycloids or self.sc.many_changed or self.sc.cycloid_changed:
    ...
    def consider_lons(self):
        ...
        if self.sc.parameters['projection'] == 'ortho' and rx - lx >= 180:
            ...
            lx = cx - 90 + 1
            rx = cx + 90 - 1
        return numpy.linspace(lx, rx, int(self.grid.lon_num*self.n_interp))
    ...
    def consider_lats(self):
        ...
        if proj == 'spaeqd' and hy > 0 and ly < 0:
            ...
            ly = cy - 90 + 1
            hy = cy + 90 - 1
        return numpy.linspace(ly, hy, int(self.grid.lat_num*self.n_interp))
    ...
    def prepare_plot_series(self):
        ...
        i,j = numpy.meshgrid(
            numpy.linspace(0, self.grid.lon_num - 1, int(self.grid.lon_num*self.n_interp)),
            numpy.linspace(0, self.grid.lat_num - 1, int(self.grid.lat_num*self.n_interp))
        )
...
class SatStressPanel(wx.Panel):
    ...
    def __init__(self, *args, **kw):
        ...
        from satstress.satstress import StressDef  
        self.sc.stressdef = StressDef()
        self.sc.stressdef.satellite = self.sc.satellite
...
class SatStressApp(wx.App):
    def OnInit(self):
        self.frame = SatStressFrame(None, title=u'SatStressGUI V6.0', size=(800,800))
...
def main():
	...
	print("Launching SatStress GUI...")
	os.environ['PATH'] += os.path.pathsep+os.path.abspath(os.curdir)
	app = SatStressApp(False) # The 0 aka false parameter means "don't redirect stdout and 	stderr to 	a window"    app.MainLoop()
	app.MainLoop()
